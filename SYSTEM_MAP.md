...
> **最后更新：** 2026-07-15（第十三阶段：画布技术全面整改）  
...
│   │
│   ├── page.tsx                    ★★★ 全新落地页：网站介绍展示（公开展示，无需登录）
│   │                                  - 全屏纵向大图案液态玻璃动态背景
│   │                                  - 顶部导航栏：LOGO + 登录按钮 + 控制台入口
│   │                                  - Hero 区 + 功能亮点卡片 + CTA
│   │                                  - 暗色调延续现有设计语言，独有垂直流态有机体动画
│   │
│   ├── login/
│   │   └── page.tsx                ★ 登录/注册页（从原 page.tsx 提取，独立路由）
│   │                                  - 登录表单 + 注册表单（邀请码制）
│   │                                  - 星系液态玻璃风格（完整保留原设计）
│   │                                  - 已登录自动跳转 /workspace
│   │
│   ├── workspace/
│   │   └── page.tsx                ★ 工作台 SPA（原 page.tsx 的已认证全部内容）
...
│   │   └── AssetDock.tsx           侧边素材坞（拖入历史生图/生视频）
│   ├── chat/                       AI 对话模块
│   ├── image-gen/                  图片生成模块
│   ├── video-gen/                  视频生成模块
│   ├── workflow/                   工作流/智能体中心
...
| `canvasSettings` | object | 画布全局设置（defaultImageModel、defaultVideoModel、globalPromptSuffix、globalRatio、directorGenre、directorTempo） |
...
| 18 | 提示词相关优化（场景表/面孔识别/三视图后缀） | 影响生成质量 | 待后续阶段实施 |
...
> **Owner 提示：** 如需修改画布逻辑，优先关注 `VideoCanvas.tsx`、`CustomNodes.tsx`、`useCanvasEngine.ts`、`useAppStore.ts`。如需修改路由/登录/工作台，关注 `app/page.tsx`（落地页）、`app/login/page.tsx`（登录页）、`app/workspace/page.tsx`（工作台SPA）。同步/存储关注 `main.py`（sync_sessions + 媒体存储 + 用户体系）、`director-rules.ts`（导演引擎）、`dialogStore.ts`（统一弹窗）、`DialogManager.tsx`（弹窗UI）、`SettingsModal.tsx`（API配置/Admin面板）、`services/api.ts`（全局拦截器）共 13 个文件。
